...
export class AdminEventPage implements OnInit, OnDestroy {
  ...
  async save() {
    ...
    await this.updateEventService.mutate({
        ...
        minimumCapacity: this.minimumCapacity?.value && Number(this.minimumCapacity?.value),
        maximumCapacity: this.maximumCapacity?.value && Number(this.maximumCapacity?.value),
